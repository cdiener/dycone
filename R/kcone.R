...
plot_red = function(basis_list, arrows = TRUE, col = NULL, n_cl = NULL) {
    ...
    if (max(b_cols) > 1000 || !is.null(n_cl)) {
        ...
        cl = lapply(red, function(b) suppressWarnings(kmeans(b, centers = n_cl, iter.max = 100, 
            nstart = 3)))
...
hyp = function(normal, disease, reacts, type = "transformation", correction_method = "fdr", 
    ...
    if (type == "transformation") {
        ...
        if (requireNamespace("foreach", quietly = TRUE)) {
            i = 1:ncol(M)
            opt = foreach::"%dopar%"(foreach::foreach(i = i, .combine = cbind), dba(v_opt, 
                S, M[, i], lower = 0, upper = 1))
        } else {
